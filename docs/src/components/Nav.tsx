const BASE = import.meta.env.BASE_URL; // "/agent-conductor/"

const LINKS = [
  { key: "board", label: "Board", href: `${BASE}#/board` },
...
/**
 * One nav, used everywhere — the SPA routes (hash links, so they navigate
 * client-side and animate) and the standalone Board guide page. `active`
      ...
      <nav className="mx-auto flex h-14 max-w-5xl items-center justify-between px-5">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {LINKS.map((l) => (
            <a
              ...
              className={`rounded-lg px-3 py-2 text-sm transition-colors ${
                active === l.key ? "text-chalk" : "text-mist hover:text-chalk"
              }`}
            >
              {l.label}
            </a>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group ml-2 flex items-center gap-2 rounded-lg border border-line-2 bg-panel px-3 py-2 text-sm text-chalk transition-all duration-200 hover:-translate-y-px hover:border-mist/40 hover:bg-panel-2"
          >
            <svg width="15" height="15" viewBox="0 0 16 16" fill="currentColor" aria-hidden className="transition-transform duration-200 group-hover:scale-110">
