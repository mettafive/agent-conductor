    ...
    <div className="mx-auto max-w-2xl px-6 py-8">
      <div className="text-center">
        ...
        <p className="mt-1 font-mono text-xs text-mist">
          {model.unitsDone}/{model.unitsTotal} units · {model.insightCount} insight
          {model.insightCount === 1 ? "" : "s"}
          {model.suggestions.length > 0 && ` · ${model.suggestions.length} suggestion${model.suggestions.length === 1 ? "" : "s"}`}
        </p>
        {model.suggestions.length > 0 && onOpenInsights && (
          <button
            onClick={onOpenInsights}
            className="mt-3 rounded-lg border border-iris/40 bg-iris/10 px-3 py-1.5 font-mono text-[11px] text-iris transition-colors hover:bg-iris/15"
          >
            ✨ Review {model.suggestions.length} optimization{model.suggestions.length === 1 ? "" : "s"}
          </button>
        )}
      </div>
      ...
      <div className="mt-6 space-y-1.5">
        {model.steps.map((s) => (
          ...
          >
            <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${DOT[s.column] ?? "bg-line-2"}`} />
            <span className="min-w-0 flex-1 truncate font-mono text-[12px] text-chalk">{s.id}</span>
            {s.isLoop && s.loop && (
              <span className="shrink-0 font-mono text-[10px] text-iris">
            ...
            {s.heartbeat.some((h) => h.insight) && <span className="shrink-0 text-[11px]">💡</span>}
          </div>
        ))}
      </div>
    </div>
